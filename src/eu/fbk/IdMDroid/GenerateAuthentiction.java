...
import eu.fbk.IdMDroid.IDMP.DB.Auth0;
 ...
 */
public class GenerateAuthentiction extends AnAction {
    private InsertCodeHandler insertCodeHandler;
    ...
    public void actionPerformed(AnActionEvent e) {
        ...
        GenerateDialogAuthentication dlg = new GenerateDialogAuthentication (psiclass);
        dlg.show();
        String idpName = GenerateClassAuthentication.getInstance().getIdmName();
        if ((idpName.equals("Auth0") || idpName.equals("OKTA") || idpName.equals("Microsoft")) && (!GenerateClassAuthentication.getInstance().isHasRedirect()) && (GenerateClassAuthentication.getInstance().getAppID().equals("")) && (GenerateClassAuthentication.getInstance().getBtnName().equals("")) && (GenerateClassAuthentication.getInstance().getDevDomain().equals("")))
            {
                if (dlg.isOK()) {
        ...
        } else if (idpName.equals("Google") && (!GenerateClassAuthentication.getInstance().isHasRedirect()) && (GenerateClassAuthentication.getInstance().getAppID().equals("")) && (GenerateClassAuthentication.getInstance().getBtnName().equals(""))) {
            if (dlg.isOK()) {
                Messages.showErrorDialog("Please complete the needed information ", "Please, Fill The Mandatory Fields");
            }
        } else if (idpName.equals("Yahoo") && (!GenerateClassAuthentication.getInstance().isHasRedirect()) && (GenerateClassAuthentication.getInstance().getAppID().equals("")) && (GenerateClassAuthentication.getInstance().getBtnName().equals("")) && (GenerateClassAuthentication.getInstance().getAppSecret().equals(""))) {
            if (dlg.isOK()) {
                Messages.showErrorDialog("Please complete the needed information ", "Please, Fill The Mandatory Fields");
            }
        } else {
    ...
    private void generateSupportiveClasses() {
       ...
       // String oidchWithRefTokentemplate = "TokenActivityOIDCWithRefTokenTemplate.java";
        ...
        JavaDirectoryService.getInstance().createClass(psiDirectory, GenerateClassAuthentication.getInstance().getClassNameAuth(), authStateManagerTemplate, true);
        JavaDirectoryService.getInstance().createClass(psiDirectory, GenerateClassAuthentication.getInstance().getClassNameConfiguration(),configTemplate , true);
        JavaDirectoryService.getInstance().createClass(psiDirectory, GenerateClassAuthentication.getInstance().getClassNameConnectionBuilderForTesting(), connectionTemplate, true);

        if (GenerateClassAuthentication.getInstance().isHasTest()){
            //JavaDirectoryService.getInstance().createClass(psiDirectory, GenerateClassAuthentication.getInstance().getClassNameTokenOAuth(),oidchWithRefTokentemplate , true);
    ...
    private void inspectAndroidManifest() {
        ...
        if (GenerateClassAuthentication.getInstance().isHasAppLink()){
        ...
        if (GenerateClassAuthentication.getInstance().getPath()!= null){
            dataTag.setAttribute("android:scheme",GenerateClassAuthentication.getInstance().getScheme().substring(0,GenerateClassAuthentication.getInstance().getScheme().indexOf(":")));
            dataTag.setAttribute("android:host", GenerateClassAuthentication.getInstance().getHost());
            dataTag.setAttribute("android:path",GenerateClassAuthentication.getInstance().getPath());
        } else {
            dataTag.setAttribute("android:scheme",GenerateClassAuthentication.getInstance().getScheme().substring(0,GenerateClassAuthentication.getInstance().getScheme().indexOf(":")));
            dataTag.setAttribute("android:host", GenerateClassAuthentication.getInstance().getHost());
        }
     ...
     * it will add the manifestPlaceHolder equal to the custom url value. This class is not working anymore
     */
  /*  private void inspectBuildGradle() {
    ...
    private void inspectBuildGradle() throws IOException {
        ...
        if (GenerateClassAuthentication.getInstance().isHasCustom()){
            ...
            cutomUrl = GenerateClassAuthentication.getInstance().getCustomURL();
    ...
    private void inspectJSONFile() {
        String idpName = GenerateClassAuthentication.getInstance().getIdmName();
        String devDomain, discoveryURL = " ", c_ID = " ", c_Sec=" ", r_URI = " ", scope = " ";
        ...
        try {
            ...
            devDomain = GenerateClassAuthentication.getInstance().getDevDomain();
            if (devDomain != null){ //
            ...
            c_ID =  GenerateClassAuthentication.getInstance().getAppID();
            if (GenerateClassAuthorization.getInstance().isHasSecret()){
                c_Sec= GenerateClassAuthorization.getInstance().getAppSecret();
            } else{
                c_Sec=" ";
            }
            scope = GenerateClassAuthentication.getInstance().getScope();
            if(GenerateClassAuthentication.getInstance().isHasCustom()){
                r_URI = GenerateClassAuthentication.getInstance().getCustomURL();
            } else if (GenerateClassAuthentication.getInstance().isHasAppLink() & GenerateClassAuthentication.getInstance().getPath()!=null) {
                r_URI = GenerateClassAuthentication.getInstance().getScheme() + GenerateClassAuthentication.getInstance().getHost() + GenerateClassAuthentication.getInstance().getPath();
            } else {
                r_URI = GenerateClassAuthentication.getInstance().getScheme() + GenerateClassAuthentication.getInstance().getHost();
            }
    ...
    private void generateCompareTo(PsiClass psiClass) {
        ...
        StringBuilder Var8 = new StringBuilder("Button " + GenerateClassAuthentication.getInstance().getBtnName() + ";");
